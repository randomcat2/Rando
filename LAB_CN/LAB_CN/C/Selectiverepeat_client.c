...
int main() {
    ...
    while (base < num_packets) {
        for (int i = next_seq_num; i < base + WINDOW_SIZE && i < num_packets; i++) {
        ...
        for (int i = next_seq_num; i < base + WINDOW_SIZE && i < num_packets; i++) {
            ...
            if (ack_packet.seq_num == i && rand() % 2) {
                printf("Received acknowledgment for packet with sequence number %d\n", ack_packet.seq_num);
                base++;
            } else {
                printf("Received negative acknowledgment for packet with sequence number %d. Retransmitting...\n", ack_packet.seq_num);
            }
        }
